...
common_context_seq_rm_type common_context_can_seq_rm(llama_context * ctx, bool mtp) {
    ...
    if (mtp) {
        const int32_t width = llama_model_n_embd_out(llama_get_model(ctx));
        GGML_ASSERT(width > 0);
        // A token-only MTP batch leaves the graph's hidden-state input untouched.
        // This capability probe has no target activations, so supply deterministic
        // zeros rather than evaluating uninitialized allocator contents.
        hidden.assign(tmp.size() * static_cast<size_t>(width), 0.0f);
        batch.embd = hidden.data();
        COM_TRC("MTP sequence-removal probe: %zu tokens, %d zero hidden-state floats per token\n", tmp.size(), width);
    }
